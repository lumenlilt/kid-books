...
export function createCuckooClock(accent: number): CuckooClock {
  ...
  const mk = (geo: BoxGeometry | ConeGeometry | CylinderGeometry | SphereGeometry, mat = wood) => {
    const m = new Mesh(geo, mat);
    m.castShadow = true;
    m.receiveShadow = true;
    return m;
  };
  const body = mk(new BoxGeometry(0.28, 0.34, 0.12));
  body.position.set(0, 0.05, 0);
  g.add(body);
  const roof = mk(new ConeGeometry(0.23, 0.16, 4), dark);
  ...
  const hm = mk(new BoxGeometry(0.016, 0.055, 0.006), paper(0x2b2220, { flat: false }));
  hm.position.y = 0.024;
  hourHand.add(hm);
  const minuteHand = new Group();
  minuteHand.position.set(0, 0.03, 0.084);
  const mm = mk(new BoxGeometry(0.012, 0.08, 0.006), paper(0xd9534f, { flat: false }));
  mm.position.y = 0.036;
  minuteHand.add(mm);
  g.add(hourHand, minuteHand);
  // 小門與鳥
  const door = mk(new BoxGeometry(0.08, 0.07, 0.01), dark);
  ...
  const rod = mk(new BoxGeometry(0.012, 0.16, 0.008), paper(0xffd45a, { flat: false }));
  rod.position.y = -0.08;
  pendulum.add(rod);
  ...
  g.add(pendulum);
  for (const s of [-1, 1]) {
    const chain = mk(new BoxGeometry(0.006, 0.12, 0.006), dark);
